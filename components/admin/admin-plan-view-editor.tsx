    ...
    <div>
      ...
      <div className="flex items-center gap-2 mb-2 flex-wrap">
        <div className="flex items-center bg-muted/50 rounded-lg p-1 gap-1">
          <button
            className={`p-1.5 rounded-md transition-colors ${tool === 'draw' ? 'bg-white shadow-sm text-[var(--color-primary)]' : 'text-muted-foreground hover:text-foreground'}`}
            onClick={() => setTool('draw')} title="Rysuj"
          ><Pencil className="h-4 w-4" /></button>
          <button
            className={`p-1.5 rounded-md transition-colors ${tool === 'select' ? 'bg-white shadow-sm text-[var(--color-primary)]' : 'text-muted-foreground hover:text-foreground'}`}
                ...
                {currentPts.length > 0 && (
                  <g>
                    <polyline
                      ...
                      fill="none" stroke="var(--color-primary)" strokeWidth={2 * iz} strokeDasharray={`${6 * iz} ${3 * iz}`}
                    />
                    {currentPts.map((p, i) => (
                      <circle key={i} cx={p.x} cy={p.y} r={4 * iz} fill={i === 0 && nearFirst ? '#22c55e' : 'var(--color-primary)'} stroke="#fff" strokeWidth={1.5 * iz} />
                    ))}
        ...
        {viewPolygons.length > 0 && (
          <div className="flex flex-wrap gap-2 p-3 bg-muted/30 rounded-lg">
            ...
            {viewPolygons.map(poly => (
              <div
                key={poly.id}
                className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm cursor-pointer transition-colors ${selectedPolyId === poly.id ? 'bg-[var(--color-primary)]/10 text-[var(--color-primary)]' : 'bg-white border hover:bg-muted/50 text-foreground'}`}
                onClick={() => { setTool('select'); setSelectedPolyId(poly.id) }}
              >
                <span className="font-medium">{poly.refLabel}</span>
                <button className="text-muted-foreground hover:text-destructive flex-shrink-0" onClick={(e) => { e.stopPropagation(); handleDeletePoly(poly.id) }}>
        ...
        <DialogContent className="max-w-sm">
          ...
          <div className="max-h-64 overflow-y-auto space-y-1">
            ...
            {filteredItems.map(it => (
              <button
                key={it.id}
                className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors flex items-center justify-between ${mappedRefIds.has(it.id) ? 'opacity-50' : 'hover:bg-muted/50'}`}
                onClick={() => confirmPick(stageMode ? 'stage' : 'unit', it.id, it.label)}
              >
                <span className="font-medium">{it.label}</span>
                {mappedRefIds.has(it.id) && <span className="text-xs text-muted-foreground ml-2">już dodany</span>}
              </button>
            ))}
          </div>
          <DialogFooter>
  ...
  return (
    <div className="bg-white rounded-2xl border border-border p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <Button size="sm" onClick={() => setShowAddDialog(true)} style={{ backgroundColor: 'var(--color-primary)' }}>
      ...
      <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
        <DialogContent className="max-w-xs">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAddView} disabled={adding || !newViewName.trim()} style={{ backgroundColor: 'var(--color-primary)' }}>

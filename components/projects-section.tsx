...
export async function ProjectsSection() {
  ...
  const active = dbProjects.filter((p) => p.status !== "completed");
  const completed = dbProjects.filter((p) => p.status === "completed");
          ...
          activeTab={
            <div className="grid gap-8 md:grid-cols-2">
              {active.length > 0 ? (
                ...
                <p className="col-span-2 text-center py-16 text-muted-foreground">Brak aktualnych inwestycji.</p>
              )}
            </div>
          }
          completedTab={
            <div className="grid gap-8 md:grid-cols-2">
              {completed.length > 0 ? (
                ...
                <p className="col-span-2 text-center py-16 text-muted-foreground">Brak zakończonych inwestycji.</p>
              )}
            </div>
          }
